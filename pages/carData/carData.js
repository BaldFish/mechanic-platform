...
Page({
  ...
  //立即查看或打开modal
  lookDetail(e) {
    if (e.currentTarget.dataset.data.can_read && e.currentTarget.dataset.data.is_vip) {
      app.data.manualId = e.currentTarget.dataset.data.manual_id;
      wx.navigateTo({
        url: `/pages/read/read?previousPage=carData`
      });
    } else if (e.currentTarget.dataset.data.is_vip) {
      let data = {
    ...
    } else if (e.currentTarget.dataset.data.can_read) {
      app.data.manualId = e.currentTarget.dataset.data.manual_id;
      wx.showModal({
        title: "提示",
        content: "您已成功支付订单",
  ...
  getPointsBalance() {
    app.util.request(
      "GET",
      `/v1/rrd-wx-app/user/points/balance/${app.data.address}`,
      "application/json",
      "",
      `${app.data.token}`,
      res => {
  ...
  toPay(e) {
    ...
    if (this.data.seleted === "2") {
      ...
      data.points_amount =
        this.data.pointsBalance >= 500 ? 500 : this.data.pointsBalance;
    }
    app.util.request(
      "POST",
      `/v1/rrd-wx-app/order`,
